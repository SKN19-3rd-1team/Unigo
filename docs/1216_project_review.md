...
* **물리적 분리**: `SA_MAJOR`(SQLAlchemy)와 `APP_MAJOR`(Django)는 이름이 비슷하지만 **완전히 다른 테이블**(`majors` vs `unigo_app_major`)입니다.
* **현재 상태**: `seed_all.py` 등 핵심 스크립트는 `SA_MAJOR`(`majors`) 테이블에 데이터를 넣고 있습니다. `APP_MAJOR` 테이블들은 사용되지 않거나 비어있을 가능성이 높습니다.
