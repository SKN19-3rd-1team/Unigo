...
class Major(models.Model):
    ...
    name = models.CharField(max_length=255, unique=True, help_text="전공명")
    cluster = models.CharField(
        max_length=100, help_text="계열 (예: 공학계열, 인문계열)"
    )
    ...
    employment_rate = models.CharField(
        max_length=50, null=True, blank=True, help_text="취업률"
    )
    department = models.TextField(help_text="관련 학과명 (쉼표로 구분)")

    # JSON 필드
    relate_subjects = models.JSONField(help_text="관련 과목 정보")
    career_activities = models.JSONField(null=True, blank=True, help_text="진로 활동")
    jobs = models.TextField(help_text="진출 직업 (쉼표로 구분)")
    qualifications = models.TextField(
        null=True, blank=True, help_text="관련 자격증 (쉼표로 구분)"
    )
    enter_fields = models.JSONField(null=True, blank=True, help_text="진출 분야 정보")
    main_subjects = models.JSONField(null=True, blank=True, help_text="주요 과목 정보")
    chart_data = models.JSONField(
        null=True, blank=True, help_text="통계 데이터 (성별, 분야, 연봉, 만족도 등)"
    )
...
class University(models.Model):
    ...
    name = models.CharField(max_length=255, help_text="대학명")
    area = models.CharField(max_length=100, help_text="지역")
    campus_name = models.CharField(max_length=255, help_text="캠퍼스명")
    school_url = models.URLField(
        max_length=500, null=True, blank=True, help_text="대학 홈페이지 URL"
    )
...
class MajorUniversity(models.Model):
    ...
    major = models.ForeignKey(
        Major, on_delete=models.CASCADE, related_name="universities"
    )
    university = models.ForeignKey(
        University, on_delete=models.CASCADE, related_name="majors"
    )
    major_name_at_university = models.CharField(
        max_length=255, help_text="해당 대학에서의 전공명 (대학마다 다를 수 있음)"
    )
