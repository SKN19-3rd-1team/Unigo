...
### 시연 페이지
#### tool calling 
- 질문 예시: 나는 코딩에 관심이 있어. 관련 학과를 추천해줘.
1. list_departments: 학과 목록 조회 

- 질문 예시: 해당 학과가 있는 대학교들을 알려줘.
2. get_universities_by_department: 특정 학과가 있는 대학 조회

- 질문 예시: ~대학교의 ~학과를 졸업하면 무슨 일을 해?
...
- 질문 예시: 데이터셋별로 잘 갖고 오는지

- 질문 예시: 지난 대화를 저장하는데 이를 토대로 답을 하는건지

- 질문 에시: 학과들 관련해서 추천해오는 학교들이 랜덤하게 불러오는것인지

- 질문 예시: 서울대 컴공대, 홍익대 컴공과, 나는 천문학과가 가고싶어-> 어떤 학과이고, 어디 대학에 있으며, 졸업 후 진로와 연봉은 어떤가요?를 
동시에 불러오면서 오류가 발생
